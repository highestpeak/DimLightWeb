    ...
    >
      ...
      <template #cell(image)="data">
        <b-avatar
          ...
          :style="'background-color:' + data.value.imageColor"
          size="2rem"
        ></b-avatar>
      </template>
      ...
      <template #cell(tags)="data">
        <b-badge
          v-for="(type, index) in data.value.slice(0, 5)"
          :key="index"
...
<script>
import { getFeedList, delRSSSource } from "@/util/request";
export default {
  name: "feed-list",
  data() {
    return {
  ...
  methods: {
    ...
    // eslint-disable-next-line no-unused-vars
    onFeedListFetchSucceed(feedList){
    ...
    delRssSource(row){
      ...
      delRSSSource(rssSource,function() {
        ...
        vueApp.totalRows =  vueApp.items.length;
      })
    }
  },
};
